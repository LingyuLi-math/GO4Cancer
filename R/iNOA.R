rm(list=ls())
cedge <-as.matrix(read.table("kminter\\cedge10.txt"))
upfile = cedge
NOA <- function(upfile,species="Human",idinput,reffile=NULL,evidences="ALL"){
...
result <- NOA(cedge,species = "Human","SYMBOL",reffile=NULL,evidences="ALL")
cjprad10sim0.8 <-result$noaresult$term
write.csv(njinter10sim_0.8,"D:\\various cancer genes\\Hepatocellular carcinoma\\keggmalainter\\njinter10sim_0.8.csv")
...
edge.test <- function(termtable,reftermtable,testnet,refnet,Ontology){
  ...
  tnamesim <-as.matrix(mgoSim(tname,tname,semData=d,measure = "Wang",combine=NULL))
  #diag(tnamesim) <-0
  #tnamesim[lower.tri(tnamesim)]=0 
  DF = tnamesim
  value = 0.8
  ...
  go1go2 <-as.matrix(which.names(DF,value))
  a<- strsplit(go1go2," ")
  go1<- sapply(a,"[",1)
  go2<- sapply(a,"[",2)
  go1go21<- as.matrix(cbind(go1,go2))
  colnames(go1go21) <-NULL
  termn <- length(tname)
  ...
  for(i in 1:termn){
    e <-c() 
    for (q in 1:nrow(go1go21)) {
      if(go1go21[q,1]==tname[i]) {
        ter <-go1go21[q,2]
        e = rbind(e,ter)
      }
    }
    genetest <-c()
    for (t in 1:length(e)) {
      genetest1 <-as.matrix(unique(termtable[termtable[,2]==e[t],1]))
      genetest <-unique(rbind(genetest,genetest1))
    }
    x <- sum(testnet$matrix[genetest,genetest])/2
    #generef <- unique(reftermtable[reftermtable[,2]==tname[i],1])
    X <- sum(refnet$matrix[genetest,genetest])/2
    if(x!=0){
      kterm <- kterm + 1
...
read.file <- function(upfile,reffile){
 ...
 anaset$genepair <- toupper(upfile)
...
read.net <- function(file){
	net.text <- as.matrix(read.table("D:\\various cancer genes\\Prostate cancer\\kminter\\cedge10.txt", fill=T, as.is=T, col.names=1:max(count.fields("D:\\various cancer genes\\Prostate cancer\\kminter\\cedge10.txt"))))
