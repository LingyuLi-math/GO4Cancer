...
uppfile <- read.table('cancer pathway genes/Cedge.txt',header = FALSE)
uppfile
zz <-as.matrix(uppfile)
zz
species="Human"
upfile = zz
idinput = "SYMBOL"
...
result <- NOA(zz,species = "Human","SYMBOL",reffile=NULL,evidences="ALL")
print(result$noaresult)
print(result$noaresult$term)
...
edge.test <- function(termtable,reftermtable,testnet,refnet,Ontology){
 ...
 # sum(testnet$matrix[genetest,genetest])
  for(i in 1:termn){
    eee <-termtable[termtable[,2]==tname[i],1]}
...
read.net <- function(file){
  net.text <- as.matrix(read.table('LIHC edge.txt', fill=T, as.is=T, col.names=1:max(count.fields('LIHC edge.txt'))))
